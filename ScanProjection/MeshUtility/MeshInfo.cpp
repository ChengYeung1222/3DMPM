...
bool TrglCmp(const Mesh::TRGL &lhs, const Mesh::TRGL &rhs)
{
#if 1
	unsigned int l[3], r[3];
	memcpy(l, &lhs, 12);
	sort(l, l + 3);
	memcpy(r, &rhs, 12);
	sort(r, r + 3);
...
bool TrglEqual(const Mesh::TRGL &lhs, const Mesh::TRGL &rhs)
{
	unsigned int l[3], r[3];
	memcpy(l, &lhs, 12);
	sort(l, l + 3);
...
void MeshInfo::MakeRegular(uint *maps)
{
	...
	sort(indicesBucket, indicesBucket + nTriangles, TrglCmp);
	for (int i = 0; i < nTriangles; i++) {
		const Mesh::TRGL &srcTri = indicesBucket[i];
		if (!TrglEqual(srcTri, token)) {
		//if (srcTri != token) {
			token = srcTri;
...
//if ((curEdge.i == 118 && curEdge.j == 58) || (curEdge.i == 58 && curEdge.j == 118))
//int dummy=1;
		if (curEdge != token) {
...
#else // In this version, redundant triangles are eliminated first rather than
	  // the edges.
bool operator < (const TriInfo &l, const TriInfo &r)
{
...
bool operator == (const TriInfo &l, const TriInfo &r)
{
	...
	return (il[0] == ir[0] && il[1] == ir[1] && il[2] == ir[2]);
}


void MeshInfo::Extract(uint *maptable)
{
	MakeRegular(maptable);
	...
	for (int i = 0; i < nTriangles; i++) {
		...
		int eBase = i * 3;
		TriInfo tri = { (uint *) ind, { -1, -1, -1 }, DPoint3(a0, a1, a2), 
					    curNorm };
		m_triinfo.push_back(tri);
	}
...
#if KENGDIE_DEBUG
DPoint3 *addr = &m_vertices[0];
const TriInfo *addrTriInfo = &m_triinfo[0];
const EdgeInfo *addrEdgeInfo = &m_edgeinfo[0];
FILE *fp = fopen("kengdie.vs", "w");
const char *sHeader = "GOCAD VSet 1\n"
						"HEADER{\n"
						"name:kengdie\n"
						"}\n";
fprintf(fp, sHeader);
...
if (countSingleEdge != 0) {
	...
	// reset triangle information with respect to the edges
	for (int i = 0; i < nEdges; i++) {
		const EdgeInfo &curEdge = m_edgeinfo[i];
	...
	// set Triangles for each vertex
	for (int i = 0; i < nTriangles; i++) {
		TriInfo *pTri = &m_triinfo[i];
		uint *ind = pTri->ind;
	...
	while (!eQueue.empty()) {
		...
		if (info.state == USE_ALPHA_AND_BETA && !visited[curTriLoc]) {
			...
			if (fabs(curProd - realProd[loc]) > 1.0e-6) {
				...
				std::swap(curTriInfo.ind[0], curTriInfo.ind[2]);
				cout << curTriLoc << endl;
			}

			OtherTwo(curTriInfo, loc, other0, other1);
//if (other0 ==-1 || other1 == -1)
//OtherTwo(curTriInfo, loc, other0, other1);
			if (other0 >= 0)
				eQueue.push(other0);
			if (other1 >= 0)
